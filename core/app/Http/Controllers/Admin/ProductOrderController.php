...
class ProductOrderController extends Controller
{
    ...
    public function settings() {
        ...
        return view('admin.product.order.settings', $data);
    }
    ...
    public function updateItemStatus(Request $request)
{
    ...
    if ($item) {
        ...
        } elseif ($previousStatus === 'canceled' && $status === 'approved') {
            // If the previous status was 'cancelled' and the new status is 'approved', add the OrderItem total back to the ProductOrder total
            $newTotal = $productOrder->total + $item->total;
    ...
    public function timeframeStore(Request $request) {
        $rules = [
            'start' => 'required',
            'end' => 'required',
            'max_orders' => 'nullable|numeric|min:1',
        ];
