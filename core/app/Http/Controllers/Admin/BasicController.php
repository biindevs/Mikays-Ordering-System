...
class BasicController extends Controller
{
    ...
    {
        $pwa = file_get_contents('manifest.json');
    ...
    public function updatePwa(Request $request)
    {
        ...
        $preManifest = file_get_contents('manifest.json');
        $preManifest = json_decode($preManifest, true);

        if ($request->hasFile('icon_128')) {
            $ext = $icon128->getClientOriginalExtension();
            $filename = uniqid() . '.' . $ext;
            $icon128->move('assets/front/img/', $filename);
        ...
        if ($request->hasFile('icon_256')) {
            $ext = $icon256->getClientOriginalExtension();
            $filename = uniqid() . '.' . $ext;
            $icon256->move('assets/front/img/', $filename);
        ...
        if ($request->hasFile('icon_512')) {
            $ext = $icon512->getClientOriginalExtension();
            $filename = uniqid() . '.' . $ext;
            $icon512->move('assets/front/img/', $filename);
        ...
        file_put_contents('manifest.json', $content);
