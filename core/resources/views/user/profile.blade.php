                                ...
                                <div class="edit-info-area">
                                    <form action="{{route('user-profile-update')}}" method="POST"
                                        ...
                                        <div class="upload-img">
                                            @if (strpos($user->photo, 'facebook') !== false || strpos($user->photo,
                                            'google'))
                                            <div class="img-box">
                                                <img class="showimage"
                                            ...
                                            <div class="img-box">
                                                <img class="showimage"
                                                    src="{{$user->photo ? asset('assets/front/img/user/'.$user->photo) : asset('assets/front/img/user/profile.jpg')}}"
                                                    alt="user-image">
                                            </div>
                                            @endif
                                        ...
                                        <div class="row">
                                            <div class="col-lg-6">
                                                <input type="text" class="form_control"
                                                    ...
                                                    value="{{$user->fname}}" value="{{Request::old('fname')}}">
                                                @error('fname')
                                            ...
                                            <div class="col-lg-6">
                                                <input type="text" class="form_control"
                                                    placeholder="{{__('Last Name')}}" name="lname"
                                                    value="{{$user->lname}}" value="{{Request::old('lname')}}">
                                                @error('lname')
                                                <p class="text-danger mb-2">{{ $message }}</p>
                                                @enderror
                                            </div>
                                            <div class="col-lg-6">
                                                <input type="email" class="form_control" placeholder="{{__('Email')}}"
                                                    name="email" disabled value="{{$user->email}}"
                                                    value="{{Request::old('email')}}">
                                            </div>
                                            <div class="col-lg-6">
                                                <input type="text" class="form_control" placeholder="{{__('Username')}}"
                                            ...
                                            <div class="col-lg-6">
                                                <input type="text" class="form_control" placeholder="{{__('Phone')}}"
                                            ...
                                            <div class="col-lg-6">
                                                <select class="form-control" name="city">
                                                    ...
                                                    @foreach (json_decode(file_get_contents(base_path('/json/city.json')), true)['cities'] as $city)
                                                        <option value="{{ $city['value'] }}" {{ $user->city === $city['value'] ? 'selected' : '' }}>{{ $city['name'] }}</option>
                                                    @endforeach
                                                </select>
                                                @error('city')
                                                    <p class="text-danger mb-2">{{ $message }}</p>
                                            ...
                                            <div class="col-lg-6">
                                                <select class="form-control" name="state">
                                                    ...
                                                    @foreach (json_decode(file_get_contents(base_path('/json/province.json')), true)['provinces'] as $province)
                                                        <option value="{{ $province['value'] }}" {{ $user->state === $province['value'] ? 'selected' : '' }}>{{ $province['name'] }}</option>
                                                    @endforeach
                                                </select>
                                                @error('state')
                                                    <p class="text-danger mb-2">{{ $message }}</p>
                                            ...
                                            <div class="col-lg-6">
                                                <select class="form-control" name="country">
                                                    ...
                                                    @foreach (json_decode(file_get_contents(base_path('/json/barangay.json')), true)['barangays'] as $barangay)
                                                        <option value="{{ $barangay['value'] }}" {{ $user->country === $barangay['value'] ? 'selected' : '' }}>{{ $barangay['name'] }}</option>
                                                    @endforeach
                                                </select>
                                                @error('country')
                                                    <p class="text-danger mb-2">{{ $message }}</p>
                                            ...
                                            <div class="col-lg-12 mt-4">
                                                <textarea name="address" class="form_control"
                                                    placeholder="{{__('Landmark')}}">{{$user->address}}</textarea>
                                                @error('address')
                                                <p class="text-danger">{{ $message }}</p>
                                                @enderror
                                            </div>
