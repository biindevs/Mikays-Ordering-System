            ...
            <div class="col-lg-8">
                ...
                <div class="login-content">
                    ...
                    <form id="loginForm" action="{{route('user.login')}}" method="POST">
                        ...
                        <div class="input-btn">
                            <button type="submit" class="btn btn">{{__('LOG IN')}}</button>
                            <div class="mt-2 d-flex justify-content-between">
                                {{-- <a href="{{route('user-register')}}" class="mr-3">{{__("Don't have an account")}}?</a> --}}
                                <p>{{__('Don’t you have an account?')}} <a class="mr-3" href="{{route('user-register')}}">{{__('Sign up')}}</a></p>
                                <a href="{{route('user-forgot')}}">{{__('I forgot my password')}}</a>
                            </div>
                        </div>
                    </form>
                </div>
