        ...
        <div class="card">
            ...
            <div class="card-body pt-5 pb-5">
                <div class="payment-information">
                    ...
                    <div class="row mb-2">
                        <div class="col-lg-6">
                            <strong>{{__('Delivery Method')}} :</strong>
                        </div>
                        <div class="col-lg-6">
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-lg-6">
                            ...
                            {{$order->coupon}} {{$order->currency_symbol_position == 'right' ? $order->currency_symbol :
                            ''}}
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-lg-6">
                            {{$order->currency_symbol_position == 'left' ? $order->currency_symbol : ''}}
                            {{$order->tax}} {{$order->currency_symbol_position == 'right' ? $order->currency_symbol :
                            ''}}
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-lg-6">
                            ...
                            {{$order->currency_symbol_position == 'left' ? $order->currency_symbol : ''}}
                            {{$order->shipping_charge}} {{$order->currency_symbol_position == 'right' ?
                            $order->currency_symbol : ''}}
                            @else
                            {{$order->currency_symbol_position == 'left' ? $order->currency_symbol : ''}} 0
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-lg-6">
                            {{$order->currency_symbol_position == 'left' ? $order->currency_symbol : ''}}
                            {{$order->total}} {{$order->currency_symbol_position == 'right' ? $order->currency_symbol :
                            ''}}
                        </div>
                    </div>
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-lg-6">
                            ...
                            <span class="badge badge-{{$badge}} text-capitalize">{{convertUtf8(str_replace("_", " ",
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-lg-6">
                            ...
                            <button class="btn btn-info btn-sm" data-toggle="modal"
                                data-target="#modalCancellationReason">Show</button>
                            @else
                            -
                            @endif
                        </div>
                    ...
                    <!-- Modal -->
                    <div class="modal fade" id="modalCancellationReason" tabindex="-1" role="dialog"
                        aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLongTitle">Cancellation Reason</h5>
            ...
            <div class="card-body">
                <div class="table-responsive product-list">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach ($order->orderitems->whereIn('status', ['approved', 'canceled']) as $key => $item)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td><img src="{{asset('assets/front/img/product/featured/'.$item->image)}}"
                                ...
                                <td>{{$item->qty}}</td>
                                <td><span>{{$order->currency_code_position == 'left' ? $order->currency_code :
                                        ''}}</span> {{$item->total}} <span>{{$order->currency_code_position == 'right' ?
                                        $order->currency_code : ''}}</span></td>
                                @php
                                    $now = \Carbon\Carbon::now();
                                    $date = ($order->serving_method === 'pick_up') ? $order->pick_up_date :
                                    $order->delivery_date;
                                ...
                                @endphp
                                <td>
                                    <form id="itemStatusForm{{$item->id}}" class="d-inline-block" action="{{route('admin.item.status')}}" method="post">
                                        @csrf
                                        <input type="hidden" name="item_id" value="{{$item->id}}">
                                        <select class="form-control"
                                                name="item_status"
                                                onchange="document.getElementById('itemStatusForm{{$item->id}}').submit();"
                                                @if ($order->order_status === 'cancelled' || $order->completed === 'yes' || $now->gte($serving_date) || $diffInDays <= 1)
                                                disabled
                                                @endif>
                                            <option value="approved" {{$item->status == 'approved' ? 'selected' : ''}}>{{__('Approve')}}</option>
                                            <option value="canceled" {{$item->status == 'canceled' ? 'selected' : ''}}>{{__('Cancel')}}</option>
                                        </select>
                                    </form>
                                </td>
...
<script>
    ...
    function cancelItem(itemId) {
        axios.post('{{route('admin.product.item.cancel')}}', {
            _token: '{{csrf_token()}}',
            item_id: itemId
        })
