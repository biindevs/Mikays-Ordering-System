            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <div class="field-input">
                        ...
                        @error('billing_email')
                        <p class="text-danger">The Contact Email field is required.</p>
                        @enderror
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <p class="text-danger mb-2">The Country Code field is required.</p>
                    @enderror
                    @error('billing_number')
                    <p class="text-danger mb-2">The Phone field is required.</p>
                    @enderror
                </div>
...
<script>
    $(function() {
        $(".datepicker").datepicker({
            minDate: 2, // set minimum date to two days from today
            beforeShow: function(input, inst) {
                inst.dpDiv.find('.ui-state-active').removeClass('ui-state-active'); // remove active class from current date
                inst.dpDiv.find('.ui-datepicker-current-day a').addClass('ui-state-active'); // add active class to new date
            }
        }).datepicker("setDate", "+2"); // set initial date to two days from today
    });
</script>