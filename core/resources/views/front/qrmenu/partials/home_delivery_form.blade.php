        ...
        <div class="form shipping-info">
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    if(empty(old())) {
                    if (Auth::check()) {
                    $sccode = Auth::user()->shipping_country_code;
                    }
                    } else {
                    $sccode = old('shpping_country_code');
                    }
                    @endphp
                    <div class="input-group mb-3">
                        <input type="hidden" name="shpping_country_code" value="{{$sccode}}">
                        <div class="input-group-prepend">
                            <button class="btn btn-outline-secondary dropdown-toggle" type="button"
                                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{!empty($sccode) ?
                                $sccode : 'Select'}}</button>
                            <div class="dropdown-menu country-codes">
                                @foreach ($ccodes as $ccode)
                                <a class="dropdown-item" data-shpping_country_code="{{$ccode['code']}}"
                                    href="#">{{$ccode['name']}} ({{$ccode['code']}})</a>
                                @endforeach
                            </div>
                        </div>
                        <input type="text" name="shpping_number" class="form-control" value="{{$snumber}}">
                    </div>
                ...
                <div class="col-md-12">
                    ...
                    <div class="field-input">
                        ...
                        @error('shpping_city')
                        <p class="text-danger">The shipping city field is required.</p>
                        @enderror
                    </div>
                </div>

                <div class="col-md-12">
                    <div class="field-label">{{__('Barangay')}} *</div>
                    <div class="field-input">
                        @php
                        $scountry = '';
                        if(empty(old())) {
                        ...
                        <p class="text-danger mb-0">The shipping barangay field is required</p>
                        @enderror
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="field-label">{{__('Landmark')}} *</div>
                    <div class="field-input">
                        @php
                        $saddress = '';
                        if(empty(old())) {
                    ...
                    <div id="shippingCharges">
                        ...
                        <div class="form-check form-check">
                            <input class="form-check-input" type="radio"
                                ...
                                name="shipping_charge" id="scharge{{$scharge->id}}" value="{{$scharge->id}}"
                                {{$loop->first ? 'checked' : ''}}>
                            ...
                            <strong>
                                ...
                                ''}}{{$scharge->charge}}{{$be->base_currency_symbol_position == 'right' ?
                                $be->base_currency_symbol : ''}}
                            </strong>
                            @if (!empty($scharge->free_delivery_amount))
                            (@lang('Free Delivery for Orders over')
                            {{$be->base_currency_symbol_position == 'left' ? $be->base_currency_symbol :
                            ''}}{{$scharge->free_delivery_amount - 1}}{{$be->base_currency_symbol_position == 'right' ?
                            $be->base_currency_symbol : ''}})
                            @endif
                        </div>
                ...
                <div class="col-md-12">
                    <div class="form-check form-check-inline mb-3">
                        <input name="same_as_shipping" class="form-check-input ml-0 mr-2" type="checkbox"
                            id="sameAsSHipping" value="1" @guest @if(empty(old())) checked
                            @elseif(old('same_as_shipping')==1) checked @endif @endguest @auth
                            @if(old('same_as_shipping')==1) checked @endif @endauth>
                        <label class="form-check-label" for="sameAsSHipping">{{__('Billing Address will be Same as
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <div class="input-group-prepend">
                            <button class="btn btn-outline-secondary dropdown-toggle" type="button"
                                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{!empty($bccode) ?
                                $bccode : 'Select'}}</button>
                            <div class="dropdown-menu country-codes">
                                @foreach ($ccodes as $ccode)
                ...
                <div class="col-md-12">
                    ...
                    <div class="field-input">
                        ...
                        @error('billing_city')
                        <p class="text-danger">The billing city field is required.</p>
                        @enderror
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <div class="field-input">
                        ...
                        <p class="text-danger">The billing barangay field is required.</p>
                        @enderror
                    </div>
                </div>
                <div class="col-md-12">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <div class="input-group-prepend">
                            <button class="btn btn-outline-secondary dropdown-toggle" type="button"
                                ...
                                aria-expanded="false">{{!empty($user->country_code) ? $user->country_code :
                                'Select'}}</button>
                            <div class="dropdown-menu country-codes">
                                @foreach ($ccodes as $ccode)
                                <a class="dropdown-item" data-country_code="{{$ccode['code']}}"
                                    href="#">{{$ccode['name']}} ({{$ccode['code']}})</a>
                                @endforeach
                            </div>
                        </div>
                        <input type="text" name="billing_number" class="form-control"
                            placeholder="{{__('Billing Phone')}}" value="{{$bnumber}}">
                    </div>
                </div> --}}
            </div>
        </div>
        ...
        <div class="field-input">
            ...
            <select name="postal_code" class="select2">
                ...
                <option value="{{$postcode->id}}"
                    ...
                    ({{__('Delivery Charge')}} - {{$be->base_currency_symbol_position == 'left' ?
                    $be->base_currency_symbol : ''}}{{$postcode->charge}}{{$be->base_currency_symbol_position == 'right'
                    ? $be->base_currency_symbol : ''}}
                    @if (!empty($postcode->free_delivery_amount))
                    , @lang('Free Delivery for Orders over')
                    {{$be->base_currency_symbol_position == 'left' ? $be->base_currency_symbol :
                    ''}}{{$postcode->free_delivery_amount - 1}}{{$be->base_currency_symbol_position == 'right' ?
                    $be->base_currency_symbol : ''}}
                    @endif
                    )
